...
// Info shows the info on the node and apply(Tag) is just how the node's info has changed. You would need to define it manually.
// findFirst and findLast performs a clever approach (using the tree structure) to find a the first/last value that returns true when given the function f
/*
In this line:
Info operator + (const Info& a, const Info& b) {
    // write how you merge the info using the info in these 2 nodes
    // make a new node, reset the data and return the node
    return {  };
}
*/
